...
public class JwtController {
  ...
  @Operation(
      summary = "refresh-token을 받아 유효한지 체크해서 유효하면 access token 재발급",
      description = "request header에 'refresh-token'이라는 필드에 refreshToken값을 넣어서 호출")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<String> checkRefreshToken(HttpServletRequest request) {
    ...
    ResponseEntity<String> responseEntity =
        ResponseEntity.ok().headers(headers).body("Login Success!!");

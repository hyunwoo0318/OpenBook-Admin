...
public class ExamQuestionRepositoryCustomImpl implements ExamQuestionRepositoryCustom {
  ...
  @Override
  public Optional<ExamQuestion> queryExamQuestionWithDescriptionAndTopic(
      Integer roundNumber, Integer questionNumber) {
    ExamQuestion findExamQuestion =
        queryFactory
            .selectFrom(examQuestion)
            .leftJoin(examQuestion.description, description)
            .fetchJoin()
            .leftJoin(description.topic, topic)
            .fetchJoin()
            .where(examQuestion.round.number.eq(roundNumber))
            .where(examQuestion.number.eq(questionNumber))
            .fetchOne();
  ...
  @Override
  public Optional<ExamQuestion> queryExamQuestionWithDescription(
      Integer roundNumber, Integer questionNumber) {
    ExamQuestion findExamQuestion =
        queryFactory
            .selectFrom(examQuestion)
  ...
  public List<ExamQuestion> queryExamQuestionsForExamQuestionList(Integer roundNumber) {
    return queryFactory
        ...
        .leftJoin(examQuestion.description, description)
        .fetchJoin()
        .leftJoin(examQuestion.choiceList)
        .fetchJoin()
        .where(examQuestion.round.number.eq(roundNumber))

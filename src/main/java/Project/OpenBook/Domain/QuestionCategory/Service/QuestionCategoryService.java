...
public class QuestionCategoryService {
  ...
  public void updateQCNumber(List<QuestionCategoryNumberUpdateDto> dtoList) {
    List<Long> qcIdList = dtoList.stream().map(d -> d.getId()).collect(Collectors.toList());

    Map<Long, QuestionCategory> qcMap =
        questionCategoryRepository.findAllById(qcIdList).stream()
            .collect(Collectors.toMap(qc -> qc.getId(), qc -> qc));

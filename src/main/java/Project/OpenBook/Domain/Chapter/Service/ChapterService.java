...
public class ChapterService {
   ...
   * @return 저장한 단원 중복된 단원 번호 -> throw CustomException(DUP_CHAPTER_NUM);
   */
  @Transactional
  ...
  public Chapter updateChapter(int num, ChapterAddUpdateDto chapterAddUpdateDto) {
    ...
    Chapter updatedChapter =
        chapter.updateChapter(
            chapterAddUpdateDto.getTitle(),
            chapterAddUpdateDto.getNumber(),
   ...
   *
   * @param num 단원 번호
   * @return Boolean값 존재하지 않는 단원 번호 -> CHAPTER_NOT_FOUND 해당 단원에 토픽이 존재하는 경우 -> CHAPTER_HAS_TOPIC
   */
  @Transactional
  ...
  public void updateChapterNumber(List<ChapterNumberUpdateDto> chapterNumberUpdateDtoList) {
    ...
    if (idSet.size() != chapterNumberUpdateDtoList.size()
        || numberSet.size() != chapterNumberUpdateDtoList.size()) {

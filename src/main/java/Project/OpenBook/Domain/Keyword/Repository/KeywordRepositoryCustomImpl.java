...
public class KeywordRepositoryCustomImpl implements KeywordRepositoryCustom {
  ...
  public List<Keyword> queryKeywordsInTopicWithPrimaryDate(String topicTitle) {
    return queryFactory
        ...
        .leftJoin(keyword.topic, topic)
        .fetchJoin()
        .leftJoin(keyword.keywordPrimaryDateList)
        .fetchJoin()
        .where(keyword.topic.title.eq(topicTitle))
        .fetch();
  }
  ...
  public List<Keyword> queryKeywordsWithTopic() {
    return queryFactory.selectFrom(keyword).leftJoin(keyword.topic, topic).fetchJoin().fetch();
  }
    ...
    return queryFactory
        ...
        .where(
            topic
                .chapter
                .number
                .lt(answerChapter.getNumber())
                .or(
                    topic
                        .chapter
                        .number
                        .eq(answerChapter.getNumber())
